...
xmlhttp.onreadystatechange = function(){
    if(this.readyState == 4 && this.status == 200){
        ...
        sortCamerasObj.data.forEach( camera => {
            camera.merk = camera.type.toUpperCase();
            //de optische zoom moet dit ook hebben
            camera.OptischeZoom = camera.optischezoom[0];
...
let sortCamerasObj = {
    ...
    uitvoeren: function(data){
        ...
        data.forEach(camera => {
            ...
            afbeelding.setAttribute("src", camera.afbeelding);
            afbeelding.setAttribute("alt", keerDeTekstOm(camera.type));

            //maakt de titel aan
            let type = document.createElement('h3');
            type.className = "cameraSelectie__type";
            type.textContent = keerDeTekstOm(camera.type);

            //de optische-zoom
            let optisch = document.createElement('p');
            optisch.className = 'cameraSelectie__auteurs';
            //Optische Zoom tekst
            camera.optischezoom[0] = keerDeTekstOm(camera.optischezoom[0]);
            optisch.textContent =  " Optische Zoom: " + maakOpsomming (camera.optischezoom);

            //overige informatie zoals wifi en kwaliteit ect.
            let overig = document.createElement('h4');
            overig.className = "cameraSelectie__overig";
            overig.textContent = " " + camera.uitgave + " | Camera Zoom: " + camera.zoom + " | Wifi aanwezig: " + camera.wifi  +  " | Kwaliteit: " + camera.kwaliteit + " | ";
